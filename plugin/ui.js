...
const UIModule = {
    ...
    _bindEvents(){
        ...
        document.addEventListener('keydown',(e)=>{if(e.ctrlKey&&e.shiftKey&&e.key==='G'){e.preventDefault();this.toggle()}else if(e.key==='Escape'&&this._selectedRef){this._clearSelectionRef()}});
    ...
    _checkSelection(e){
        ...
        if(!sel||sel.isCollapsed||!sel.toString().trim()){
            const writeEl=document.querySelector('#write');
            if(writeEl&&sel&&sel.rangeCount&&writeEl.contains(sel.anchorNode))this._editorRange=sel.getRangeAt(0).cloneRange();
            this._clearSelectionRef();
            return;
        }
