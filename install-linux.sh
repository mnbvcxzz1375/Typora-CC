#!/usr/bin/env bash
# Typora-CC Plugin Installer — Linux
set -euo pipefail
...
TYPORA_RES="${TYPORA_RES:-/usr/share/typora/resources}"
...
if [[ ! -f "$TYPORA_HTML" ]]; then
    ...
    echo ""
    echo "  ${YELLOW}  [!] Common Linux paths:${RST}"
    echo "  ${GRAY}      /usr/share/typora/resources${RST}"
    echo "  ${GRAY}      /opt/Typora/resources${RST}"
    echo ""
    echo "  ${YELLOW}  [!] Set TYPORA_RES to your path:${RST}"
    echo "  ${GRAY}      TYPORA_RES=/path/to/resources ./install-linux.sh${RST}"
    echo ""
